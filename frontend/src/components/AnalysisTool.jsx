...
const tradeTypes = [
  'Multipliers', 'Ups & Downs', 'Touch & No Touch', 'Digits', 'Accumulators', 'Vanillas', 'Turbos'
]
...
// STRICTLY LIMITED TO THESE 13 MARKETS
...
export default function AnalysisTool() {
  ...
  const handleStartAnalysis = () => {
    setIsAnalyzing(true)
    setSelectedMarket('Scanning 13 Volatility Indices...')
    
    // Simulate deep analysis and selection of ONE of the allowed markets
    setTimeout(() => {
      const randomMarket = allowedMarkets[Math.floor(Math.random() * allowedMarkets.length)]
      setSelectedMarket(randomMarket)
    }, 2500)
  }
  ...
  return (
    <div className="p-4 space-y-4 h-full overflow-y-auto">
      <div className="mb-4">
        ...
        <p className="text-mwathe-gray text-sm">Real-time analysis across 13 Volatility Indices</p>
      </div>
      ...
      {isAnalyzing && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-mwathe-darkgray rounded-xl p-4 border border-mwathe-green/30">
          ...
          {selectedMarket && selectedMarket !== 'Scanning 13 Volatility Indices...' ? (
            <div className="space-y-2">
              ...
              <div className="w-full bg-gray-700 rounded-full h-2">
                <div className="bg-gradient-to-r from-mwathe-orange to-mwathe-green h-2 rounded-full animate-pulse" style={{width: '75%'}}></div>
              </div>
            </div>
          ) : (
            <p className="text-mwathe-gray text-sm animate-pulse">Scanning all 13 Volatility Indices for optimal entry...</p>
          )}
        </motion.div>
      )}
    </div>
