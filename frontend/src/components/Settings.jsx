    ...
    <div className="p-4 space-y-4">
      ...
      <div className="bg-mwathe-darkgray rounded-xl p-4 border border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {isDarkMode ? (
            <Moon className="text-mwathe-skyblue" size={20} />
          ) : (
            <Sun className="text-mwathe-orange" size={20} />
          )}
          <div>
        ...
        <button 
          ...
          className={`w-12 h-6 rounded-full p-1 transition-colors ${
            isDarkMode ? 'bg-mwathe-green' : 'bg-gray-600'
          }`}
        >
      ...
      <div className="bg-mwathe-darkgray rounded-xl p-4 border border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {soundsEnabled ? (
            <Volume2 className="text-mwathe-green" size={20} />
          ) : (
            <VolumeX className="text-mwathe-gray" size={20} />
          )}
          <div>
        ...
        <button 
          ...
          className={`w-12 h-6 rounded-full p-1 transition-colors ${
            soundsEnabled ? 'bg-mwathe-green' : 'bg-gray-600'
          }`}
        >
          <div className={`w-4 h-4 bg-white rounded-full transition-transform ${
      ...
      <div className="bg-mwathe-darkgray rounded-xl p-4 border border-gray-800">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button className="py-2 rounded-lg bg-mwathe-black border border-mwathe-skyblue text-mwathe-skyblue text-xs font-bold">
            Dark
          </button>
          <button className="py-2 rounded-lg bg-mwathe-black border border-gray-700 text-mwathe-gray text-xs font-bold">
            Light
          </button>
          <button className="py-2 rounded-lg bg-mwathe-black border border-gray-700 text-mwathe-gray text-xs font-bold">
            High Contrast
      ...
      <div className="bg-mwathe-darkgray rounded-xl p-4 border border-gray-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            {isConnected ? (
              <Wifi className="text-mwathe-green" size={20} />
            ) : (
            ...
            <div>
              ...
              <p className="text-mwathe-gray text-xs">
                {isConnected ? 'OAuth 2.0 Active' : 'Disconnected'}
              </p>
            </div>
          </div>
          <div className={`w-3 h-3 rounded-full ${
            isConnected ? 'bg-mwathe-green animate-pulse' : 'bg-red-500'
          }`}></div>
        </div>
        <button 
        ...
        >
          <RefreshCw size={14} />
          {isConnected ? 'Re-authorize Session' : 'Connect to Deriv'}
        </button>
      </div>
    </div>
  )
