...
export default function AutomatedBot() {
  ...
  const handleStart = () => {
    ...
    setIsRunning(true)
    addLog('✅ Parameters validated. Initializing 100+ indicators...')
    setTimeout(() => addLog('📡 Scanning all Volatility Indices for optimal entry...'), 1000)
    setTimeout(() => {
      const market = ['Volatility 10 (1s)', 'Volatility 100 (1s)', 'Volatility 75'][Math.floor(Math.random() * 3)]
      addLog(`🎯 ${market} selected. Entry point found.`)
    }, 2500)
    setTimeout(() => addLog('📈 Contract purchased. Monitoring for zero consecutive loss protection...'), 4000)
  }
