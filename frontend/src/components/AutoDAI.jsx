...
import { Play, Square, RefreshCw, Terminal, AlertCircle, TrendingUp, Target, ShieldCheck, Activity, Lock } from 'lucide-react'
...
const AI_PASSWORD = 'Denny@1249'

export default function AutoDAI({ token, accountId, onBalanceUpdate }) {
  ...
  const [targetProfit, setTargetProfit] = useState('20.00')
  const [stopLoss, setStopLoss] = useState('10.00')
  const martingaleFactor = 1.5
  const [isRunning, setIsRunning] = useState(false)
  const [validationError, setValidationError] = useState('')
  const [logs, setLogs] = useState(['AI Core initialized.'])
  ...
  useEffect(() => { if (!isRunning) setCurrentStake(parseFloat(stake) || 0.35) }, [stake, isRunning])

  const handleLogin = () => { if (password === AI_PASSWORD) { setIsAuthenticated(true); setPasswordError('') } else setPasswordError('Invalid password') }

  useEffect(() => {
    if (!isAuthenticated || !token || !accountId) return
    const connectWS = async () => {
      try {
        ...
        websocket.onopen = () => { addLog('✅ AI Connected to Deriv'); websocket.send(JSON.stringify({ balance: 1, subscribe: 1, req_id: 1 })) }
  ...
  }, [isAuthenticated, token, accountId])
  ...
  const executeTrade = () => {
    ...
    addLog(`🚀 AI Trade: ${contractType} ${durationValue}${timeframeUnit[0]} $${currentStake}`)
    ...
    if (tradeType === 'Digits' && subTradeType === 'Over/Under') proposal.barrier = option === 'Over' ? '2' : '8'
    ws.send(JSON.stringify(proposal))
  }

  const handleContractResult = (profit) => {
    const isWin = profit > 0; const newTotal = totalTrades + 1; setTotalTrades(newTotal)
    if (isWin) { setWins(wins + 1); setConsecutiveLosses(0); setCurrentStake(parseFloat(stake)); addLog(`✅ AI WON +$${profit.toFixed(2)}`) }
    else { setLosses(losses + 1); setConsecutiveLosses(consecutiveLosses + 1); const newStake = currentStake * martingaleFactor; setCurrentStake(newStake); addLog(`❌ AI LOST -$${profit.toFixed(2)}`) }
    ...
    if (isRunningRef.current) { addLog(`P/L: $${newPL.toFixed(2)} | Trades: ${newTotal}`); addLog(`⏳ AI analyzing next trade...`); nextTradeTimer.current = setTimeout(() => { if (isRunningRef.current && ws?.readyState === WebSocket.OPEN) executeTrade() }, 2000) }
  }

  const startBot = () => { setValidationError(''); if (!ws || ws.readyState !== WebSocket.OPEN) { setValidationError('Not connected.'); return }; if (nextTradeTimer.current) clearTimeout(nextTradeTimer.current); setIsRunning(true); isRunningRef.current = true; setCurrentPL(0); setTotalTrades(0); setWins(0); setLosses(0); setCurrentStake(parseFloat(stake)); setConsecutiveLosses(0); setLogs([]); addLog(` AutoD AI Started`); addLog(`Market: ${selectedMarket}`); addLog(`Target: $${targetProfit} | Stop: $${stopLoss}`); const symbol = SYMBOL_MAP[selectedMarket]; ws.send(JSON.stringify({ ticks: symbol, subscribe: 1, req_id: Date.now() })); ws.send(JSON.stringify({ ticks_history: symbol, count: 50, end: 'latest', style: 'ticks', req_id: Date.now() + 1 })); nextTradeTimer.current = setTimeout(() => { if (isRunningRef.current) executeTrade() }, 3000) }
  const stopBot = () => { setIsRunning(false); isRunningRef.current = false; if (nextTradeTimer.current) { clearTimeout(nextTradeTimer.current); nextTradeTimer.current = null }; if (ws) ws.send(JSON.stringify({ forget: 'all', req_id: Date.now() })); addLog(`⏹️ AI Stopped`) }
  const resetBot = () => { stopBot(); setLogs(['AI reset.']); setCurrentPL(0); setTotalTrades(0); setWins(0); setLosses(0); setConsecutiveLosses(0); setCurrentStake(parseFloat(stake)); setValidationError(''); setCurrentContract(null) }
  ...
  if (!isAuthenticated) {
    return (
      <div className="h-full flex items-center justify-center bg-gray-950 p-4">
        <div className="bg-gray-900 rounded-xl p-6 border border-orange-500/30 w-full max-w-sm">
          <div className="w-16 h-16 bg-orange-500/20 rounded-full flex items-center justify-center mx-auto mb-4"><Lock className="text-orange-500" size={32} /></div>
          <h2 className="text-xl font-bold text-white mb-2 text-center">AutoD AI Security</h2>
          <p className="text-gray-400 text-sm mb-6 text-center">Enter password to access AI core.</p>
          <input type="password" value={password} onChange={(e) => { setPassword(e.target.value); setPasswordError('') }} className="w-full bg-black border border-gray-700 rounded-lg px-4 py-3 text-white text-center text-lg mb-3 focus:border-orange-500 outline-none" placeholder="Enter Password" />
          {passwordError && <p className="text-red-500 text-sm mb-3 text-center">{passwordError}</p>}
          <button onClick={handleLogin} className="w-full py-3 rounded-xl bg-gradient-to-r from-orange-500 to-green-500 text-white font-bold">Unlock AI</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950 text-white p-2 overflow-hidden">
      <div className="flex items-center gap-2 pb-1 border-b border-gray-800 mb-2 flex-shrink-0"><div className="w-7 h-7 bg-gradient-to-br from-orange-500 to-green-500 rounded-lg flex items-center justify-center"><Activity size={16} className="text-white" /></div><div><h2 className="text-base font-bold text-white">AutoD AI</h2><p className="text-[10px] text-gray-400 flex items-center gap-1"><ShieldCheck size={10} /> Zero Consecutive Losses</p></div></div>
