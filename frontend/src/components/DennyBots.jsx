...
export default function DennyBots({ token, accountId, onBalanceUpdate }) {
        ...
        websocket.onmessage = (message) => {
          try {
            ...
            if (msg.msg_type === 'proposal') {
              if (msg.error) addLog(`❌ Proposal failed`)
              else setTimeout(() => websocket.send(JSON.stringify({ buy: msg.proposal.id, price: msg.proposal.ask_price, req_id: Date.now() })), 200)
            }
            if (msg.msg_type === 'buy') {
              if (msg.error) { addLog(`❌ Buy failed`); setCurrentContract(null) }
              else { setCurrentContract({ id: msg.buy.contract_id }); addLog(`✅ Contract: ${msg.buy.contract_id}`); websocket.send(JSON.stringify({ proposal_open_contract: msg.buy.contract_id, subscribe: 1, req_id: Date.now() })) }
            }
            if (msg.msg_type === 'proposal_open_contract') {
              if (!msg.error && msg.proposal_open_contract) {
                const contract = msg.proposal_open_contract
                if (contract.is_sold) { handleContractResult(parseFloat(contract.profit)); setCurrentContract(null) }
                else setCurrentPL(parseFloat(contract.profit || 0))
              }
            }
  ...
  const handleContractResult = (profit) => {
    ...
    if (isRunningRef.current) {
      ...
      nextTradeTimer.current = setTimeout(() => { if (isRunningRef.current && ws?.readyState === WebSocket.OPEN) executeTrade() }, 2000)
    }
  }

  const startBot = () => {
    setValidationError('')
    if (!ws || ws.readyState !== WebSocket.OPEN) { setValidationError('Not connected.'); return }
    if (nextTradeTimer.current) clearTimeout(nextTradeTimer.current)
    setIsRunning(true); isRunningRef.current = true
    setCurrentPL(0); setTotalTrades(0); setWins(0); setLosses(0); setCurrentStake(parseFloat(stake)); setConsecutiveLosses(0); setLogs([])
    ...
    nextTradeTimer.current = setTimeout(() => { if (isRunningRef.current) executeTrade() }, 3000)
  }

  const stopBot = () => {
    setIsRunning(false); isRunningRef.current = false
    if (nextTradeTimer.current) { clearTimeout(nextTradeTimer.current); nextTradeTimer.current = null }
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950 text-white p-2 overflow-hidden">
      <div className="flex items-center gap-2 pb-1 border-b border-gray-800 mb-2 flex-shrink-0">
        <div className="w-7 h-7 bg-gradient-to-br from-orange-500 to-green-500 rounded-lg flex items-center justify-center"><Activity size={16} className="text-white" /></div>
        <div><h2 className="text-base font-bold text-white">Denny Bots</h2><p className="text-[10px] text-gray-400 flex items-center gap-1"><ShieldCheck size={10} /> Zero Consecutive Losses</p></div>
      </div>
      ...
      <div className="bg-black rounded-lg border border-gray-800 overflow-hidden flex-1 min-h-0 flex flex-col">
        <div className="bg-gray-900 px-2 py-1 flex items-center gap-1 border-b border-gray-800 flex-shrink-0"><Terminal size={10} className="text-green-500" /><span className="text-[10px] text-gray-400 font-bold">LOG</span></div>
        <div ref={logRef} className="flex-1 p-2 overflow-y-auto font-mono text-[10px] space-y-0.5">
          {logs.map((log, i) => <p key={i} className={log.includes('✅') || log.includes('WON') ? 'text-green-400' : log.includes('❌') || log.includes('LOST') ? 'text-red-500' : log.includes('🎯') || log.includes('🚀') ? 'text-sky-400' : 'text-gray-400'}>{log}</p>)}
        </div>
      </div>
    </div>
  )
