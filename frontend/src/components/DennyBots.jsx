...
export default function DennyBots({ token, accountId, onBalanceUpdate }) {
  ...
  const handleContractResult = (profit) => {
    ...
    if (isWin) {
      ...
      // ZERO CONSECUTIVE LOSSES PROTECTION
      if (newConsecutive >= 1) {
        const martingale = parseFloat(martingaleFactor) || 1.5
        const newStake = currentStake * martingale
        setCurrentStake(newStake)
        addLog(`🛡️ Martingale ${martingale}x applied. Next: $${newStake.toFixed(2)}`)
      }
  ...
  const stopBot = () => {
    ...
    isRunningRef.current = false
    clearAllTimers()
    if (wsRef.current) wsRef.current.send(JSON.stringify({ forget: 'all', req_id: Date.now() }))
    addLog(`⏹️ Stopped`)
  }
        ...
        <div ref={logRef} className="flex-1 p-2 overflow-y-auto font-mono text-[10px] space-y-0.5">
          {logs.map((log, i) => (
            <p key={i} className={
              ...
              log.includes('️') || log.includes('') ? 'text-mwathe-skyblue' :
              'text-mwathe-gray'
            }>
              {log}
            </p>
