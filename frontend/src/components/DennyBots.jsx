...
export default function DennyBots() {
  ...
  return (
    <div className="h-full flex flex-col bg-mwathe-black text-mwathe-white p-3">
      {/* Header */}
      <div className="flex items-center gap-2 pb-2 border-b border-gray-800 mb-3">
        <div className="w-8 h-8 bg-gradient-to-br from-mwathe-orange to-mwathe-green rounded-lg flex items-center justify-center">
          <Activity size={18} className="text-white" />
        </div>
        <div>
          <h2 className="text-lg font-bold">Denny Bots</h2>
          <p className="text-xs text-mwathe-gray flex items-center gap-1"><ShieldCheck size={12} /> Zero Consecutive Loss Protection</p>
        </div>
      </div>

      {validationError && (
        <div className="bg-red-900/20 border border-red-500/50 rounded-lg p-2 mb-3 flex items-center gap-2">
      ...
      {/* PARAMETERS SECTION */}
      <div className="bg-mwathe-darkgray rounded-lg p-3 border border-gray-800 mb-3">
        <h3 className="text-mwathe-white font-bold text-sm flex items-center gap-2 mb-3"><Target size={14} className="text-mwathe-orange" /> Parameters</h3>
        
        <div className="mb-3">
          <label className="text-xs text-mwathe-gray uppercase font-bold">Select Market</label>
          <select value={selectedMarket} onChange={e => setSelectedMarket(e.target.value)} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label className="text-xs text-mwathe-gray uppercase font-bold">Trade Type</label>
            <select value={tradeType} onChange={e => setTradeType(e.target.value)} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1">
              {TRADE_TYPES.map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
          {SUB_TRADE_TYPES[tradeType]?.length > 0 && (
            <div>
              <label className="text-xs text-mwathe-gray uppercase font-bold">Sub Type</label>
              <select value={subTradeType} onChange={e => setSubTradeType(e.target.value)} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1">
        ...
        {subTradeType && OPTIONS[subTradeType] && (
          <div className="mb-3">
            <label className="text-xs text-mwathe-gray uppercase font-bold mb-1 block">Option</label>
            <div className="grid grid-cols-3 gap-2">
              {OPTIONS[subTradeType].map(opt => (
                <button key={opt} onClick={() => setOption(opt)} disabled={isRunning} className={`py-2 rounded text-sm font-bold border ${option === opt ? 'bg-mwathe-green/20 border-mwathe-green text-mwathe-green' : 'bg-mwathe-black border-gray-700 text-mwathe-gray'}`}>
                  {opt}
                </button>
              ))}
            </div>
        ...
        {tradeType === 'Digits' && subTradeType === 'Over/Under' && (
          <div className="mb-3">
            <label className="text-xs text-mwathe-gray uppercase font-bold">Predicted Digit (0-9) *</label>
            <input 
              type="number" 
              min="0" 
              max="9" 
              ...
              disabled={isRunning} 
              className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1"
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label className="text-xs text-mwathe-gray uppercase font-bold">Time Frame</label>
            {rules.fixed ? (
              <input type="text" value={rules.label} disabled className="w-full bg-mwathe-black/50 border border-gray-700 rounded px-3 py-2 text-sm mt-1 text-mwathe-gray" />
            ) : (
              <select value={timeframeUnit} onChange={e => { setTimeframeUnit(e.target.value); setDurationValue(rules.min) }} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1">
                {rules.units.map(u => <option key={u}>{u}</option>)}
              </select>
            )}
          </div>
          <div>
            <label className="text-xs text-mwathe-gray uppercase font-bold">Duration {rules.fixed ? '' : `(${rules.min}-${rules.maxMap ? rules.maxMap[timeframeUnit] : rules.max})`}</label>
            <input 
              type="number" 
              value={durationValue} 
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="text-xs text-mwathe-gray uppercase font-bold">Stake ($)</label>
            <input type="number" step="0.01" value={stake} onChange={e => setStake(e.target.value)} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1" />
          </div>
          <div>
            <label className="text-xs text-mwathe-gray uppercase font-bold">Target ($)</label>
            <input type="number" step="0.01" value={targetProfit} onChange={e => setTargetProfit(e.target.value)} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1" />
          </div>
          <div>
            <label className="text-xs text-mwathe-gray uppercase font-bold">Stop Loss ($)</label>
            <input type="number" step="0.01" value={stopLoss} onChange={e => setStopLoss(e.target.value)} disabled={isRunning} className="w-full bg-mwathe-black border border-gray-700 rounded px-3 py-2 text-sm mt-1" />
          </div>
        </div>
      </div>

      {/* LIVE PERFORMANCE DASHBOARD */}
      <div className="bg-mwathe-darkgray rounded-lg p-3 border border-mwathe-green/30 mb-3">
        <h3 className="text-mwathe-white font-bold text-sm mb-2 flex items-center gap-2">
          <TrendingUp size={14} className="text-mwathe-green" /> Live Performance
        </h3>
        <div className="grid grid-cols-4 gap-2 text-center">
          <div className="bg-mwathe-black/50 rounded p-2">
            <p className="text-xs text-mwathe-gray">Net P/L</p>
            <p className={`font-bold text-sm ${currentPL >= 0 ? 'text-mwathe-green' : 'text-red-500'}`}>
              {currentPL >= 0 ? '+' : ''}{currentPL.toFixed(2)}
            </p>
          </div>
          <div className="bg-mwathe-black/50 rounded p-2">
            <p className="text-xs text-mwathe-gray">Win Rate</p>
            <p className="text-mwathe-skyblue font-bold text-sm">{winRate}%</p>
          </div>
          <div className="bg-mwathe-black/50 rounded p-2">
            <p className="text-xs text-mwathe-gray">Trades</p>
            <p className="text-mwathe-white font-bold text-sm">{totalTrades}</p>
          </div>
          <div className="bg-mwathe-black/50 rounded p-2">
            <p className="text-xs text-mwathe-gray">Next Stake</p>
            <p className="text-mwathe-orange font-bold text-sm">{currentStake.toFixed(2)}</p>
          </div>
        </div>
        <div className="mt-2 flex items-center justify-center gap-2 bg-mwathe-black/50 rounded p-2">
          <ShieldCheck size={12} className={consecutiveLosses === 0 ? "text-mwathe-green" : "text-mwathe-orange"} />
          <p className="text-xs text-mwathe-gray">Zero Consecutive Losses: <span className="font-bold text-mwathe-white">{consecutiveLosses === 0 ? 'SECURE' : `${consecutiveLosses} Loss (Martingale Active)`}</span></p>
        </div>
      </div>

      {/* DISPLAY PANEL - Expands to fill space */}
      <div className="bg-black rounded-lg border border-gray-800 overflow-hidden flex-1 flex flex-col min-h-0 mb-3">
        <div className="bg-mwathe-darkgray px-3 py-2 flex items-center gap-2 border-b border-gray-800">
          ...
          <span className="text-xs text-mwathe-gray font-bold">DISPLAY PANEL</span>
        </div>
        <div ref={logRef} className="flex-1 p-2 overflow-y-auto font-mono text-xs space-y-1">
          {logs.map((log, i) => (
            <p key={i} className={
      ...
      {/* CONTROLS */}
      <div className="grid grid-cols-3 gap-3">
        <button onClick={startBot} disabled={isRunning} className={`py-3 rounded-lg font-bold flex items-center justify-center gap-2 text-sm ${isRunning ? 'bg-gray-800 text-gray-500' : 'bg-mwathe-green text-black'}`}>
          <Play size={16} /> Run
        </button>
        <button onClick={stopBot} disabled={!isRunning} className={`py-3 rounded-lg font-bold flex items-center justify-center gap-2 text-sm ${!isRunning ? 'bg-gray-800 text-gray-500' : 'bg-red-500 text-white'}`}>
          <Square size={16} /> Stop
        </button>
        <button onClick={resetBot} className="py-3 rounded-lg font-bold flex items-center justify-center gap-2 text-sm bg-mwathe-darkgray border border-gray-700 text-mwathe-orange">
