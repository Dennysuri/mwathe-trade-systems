...
export default function DennyBots({ token, accountId, onBalanceUpdate }) {
  ...
  const [tradeResult, setTradeResult] = useState(null) // NEW: Triggers the loop
          ...
          try {
            ...
            if (msg.msg_type === 'proposal_open_contract') {
              if (!msg.error && msg.proposal_open_contract) {
                ...
                if (contract.is_sold) {
                  setTradeResult({ profit: parseFloat(contract.profit) }) // TRIGGERS LOOP
  ...
  // THE CONTINUOUS LOOP ENGINE
  useEffect(() => {
    if (!tradeResult || !isRunning) return;

    const isWin = tradeResult.profit > 0;
    ...
    if (isWin) {
      ...
      addLog(`✅ WON +$${tradeResult.profit.toFixed(2)}`);
    } else {
      setLosses(losses + 1);
      setConsecutiveLosses(consecutiveLosses + 1);
      const newStake = currentStake * (parseFloat(martingaleFactor) || 1.5);
      setCurrentStake(newStake);
      addLog(`❌ LOST -$${tradeResult.profit.toFixed(2)}`);
    }

    const newPL = currentPL + tradeResult.profit;
    setCurrentPL(newPL);
    ...
    const timer = setTimeout(() => {
      if (isRunningRef.current) {
        executeTrade();
        setTradeResult(null); // Reset for next loop
      }
    }, 2000);

    return () => clearTimeout(timer);
  }, [tradeResult, isRunning]);

  const startBot = () => {
    setValidationError('')
    if (!ws || ws.readyState !== WebSocket.OPEN) { setValidationError('Not connected.'); return }
    setIsRunning(true); isRunningRef.current = true
    setCurrentPL(0); setTotalTrades(0); setWins(0); setLosses(0); setCurrentStake(parseFloat(stake)); setConsecutiveLosses(0); setLogs([]); setTradeResult(null)
    ...
    setTimeout(() => { if (isRunningRef.current) executeTrade() }, 3000)
  }

  const stopBot = () => { setIsRunning(false); isRunningRef.current = false; setTradeResult(null); if (ws) ws.send(JSON.stringify({ forget: 'all', req_id: Date.now() })); addLog(`⏹️ Stopped`) }
