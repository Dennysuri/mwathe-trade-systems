...
export default function AutomatedBot({ token, accountId, onBalanceUpdate }) {
  ...
  const getContractType = () => {
    if (tradeType === 'Digits') {
      ...
      return option === 'Matches' ? 'DIGITMATCH' : 'DIGITDIFF'
    }
  ...
  const calculateConfluence = (symbol, targetDigit) => {
    ...
    if (!ticks || ticks.length < 40) return { score: 30, signal: 'CALL', reasons: [] }
    let score = 40, reasons = [], signal = 'CALL'
    const digits = ticks.map(t => parseInt(t.toString().slice(-1)))
    const td = parseInt(targetDigit)
    
    if (tradeType === 'Digits') {
      if (calcFastMarkov(digits, td) > 0.40) { score += 20; reasons.push('markov') }
      if (calcFastEntropy(digits) < 2.5) { score += 15; reasons.push('entropy') }
      if (calcFastFrequency(digits, td)) { score += 15; reasons.push('frequency') }
      if (calcDormancy(digits, td) >= 20) { score += 20; reasons.push('dormancy') }
      
      signal = (option === 'Over' && td < 5) || (option === 'Under' && td > 4) || option === 'Matches' ? 'CALL' : 'PUT'
    } else {
      const hurst = calcHurst(ticks)
      if (hurst > 0.60) { score += 30; reasons.push('hurst') }
      const rsi = calcRSI(ticks)
      if (rsi < 30 || rsi > 70) { score += 25; reasons.push('rsi') }
  ...
  const scanMarkets = () => {
    ...
    Object.keys(SYMBOL_MAP).forEach(name => {
      ...
      // Elite threshold during recovery mode to prevent consecutive losses
      const requiredThreshold = recoveryModeRef.current ? 82 : 75
      
      if (res.score >= requiredThreshold && res.score > bestScore) { 
        bestScore = res.score
        bestSym = sym
        bestSignal = res.signal
  ...
  const runTradeCycle = async () => {
    if (!historyLoadedRef.current) {
      addLog('🔬 Loading fresh tick history across all markets...')
    ...
    while (isRunningRef.current) {
      try {
        ...
        // FRESH SCAN AFRESH AFTER EACH TRADE
        addLog('🔍 Scanning all markets afresh...')
        const best = scanMarkets()
        
        if (!best.symbol) {
          addLog('⏳ Waiting for high-confluence sniper signal...')
        ...
        addLog(`🎯 LOCKED: ${best.symbol} | Score: ${best.score}% [${best.reasons.join(', ')}]`)
        addLog('⚡ EXECUTING TRADE...')
        
        const proposalReq = { 
          proposal: 1, 
          amount: roundStake(currentStakeRef.current), 
          basis: 'stake', 
        ...
        if (tradeType === 'Digits' && subTradeType === 'Matches/Differs' && predictedDigit) proposalReq.barrier = predictedDigit
        ...
        if (isWin) {
          ...
          addLog(`✅ WON +$${profit.toFixed(2)} | Reset to Base Stake`)
          addLog('🟢 NORMAL TRADING MODE')
        } else {
          lossesRef.current += 1
          ...
          addLog(`🛡️ LOSS: -$${Math.abs(profit).toFixed(2)} | Blacklisted ${best.symbol}`)
          addLog(`🔴 RECOVERY MODE: Next Stake -> $${currentStakeRef.current.toFixed(2)}`)
        }
        ...
        if (sessionPLRef.current >= parseFloat(targetProfit)) { 
          addLog(`🎯 TARGET PROFIT HIT! $${sessionPLRef.current.toFixed(2)}`)
          setIsRunning(false)
  ...
  const startBot = () => {
    ...
    setLogs([])
    addLog('⚡ AUTOMATED BOT ACTIVATED (SNIPER MODE)')
